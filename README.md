# Synapse
Synapse is a distributed job scheduler that guarantees gang scheduling: it only starts training clusters when every GPU and CPU slot is available, so multi-machine model runs never sit idle waiting for a single missing node.

## Vision
AI training is brittle—64-GPU jobs fail to start unless every slot is reserved, yet generic schedulers waste compute time by partially locking resources. Synapse keeps the cluster idle until a full allocation is possible and then immediately dispatches the work, so large jobs run exactly when the hardware is ready.

## Architecture Overview
- **Scheduler (the brain)** keeps the cluster state and enforces gang scheduling. It uses gRPC, mutex-protected state, and topology hints so scheduling decisions favor nearby nodes.
- **Worker agent (the muscle)** lives on each host, registers itself, streams heartbeats, and exposes a gRPC server so the scheduler can push job start/stop commands. The Rust Carapace runtime executes the actual workloads.
- **CLI (`synctl`)** is the user interface for job submission; it calls the scheduler RPCs with desired CPU/GPU/memory requirements, and the scheduler reacts with queuing, scheduling, and dispatch logic.
- **Topology awareness** ensures requests target nodes with low-latency connectivity. The scheduler prefers nodes that are physically grouped to keep communication tight for distributed training.
